<?php	
	...
	$reserves = mysqli_query($con,"select * from `reservation` r inner join hotel h ON h.h_id = r.hotel  left join maprr as rr ON rr.r_id = r.r_id  where r.r_id in (select `r_id` from `mapur` where `Userid` = '$username') group by r.r_id  ");
